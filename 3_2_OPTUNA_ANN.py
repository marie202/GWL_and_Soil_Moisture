# HYPERPARAM OPTIMIZATION WITH OPTUNA FOR ANN
# Based on the CNN-LSTM Optuna optimization script but adapted for ANN
...
from s5_optuna_opt_ann import objective  # Import the ANN objective function
...
# Update s5_optuna_opt_ann.py to use all files
import s5_optuna_opt_ann
s5_optuna_opt_ann.INPUT_DIR = input_dir
s5_optuna_opt_ann.SELECTED_FILES_LIST = None  # Use all files, no filtering
...
# Update columns in s5_optuna_opt_ann.py
s5_optuna_opt_ann.COLUMNS_TO_KEEP = columns_to_keep
...
# Update static cols in s5_optuna_opt_ann.py
s5_optuna_opt_ann.STATIC_COLS = static_cols
...
GLOBAL_SETTINGS = {
    ...
    'model_dir_note': f"optuna_allfiles_{len(all_files)}wells_v1_ANN"
}

# --- Prepare timestamped model directory ---
timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
path = BASE_DIR / "model_runs" / "OptunaOpt"
model_dir = path / f"OptunaOpt_{timestamp}_ANN_{len(columns_to_keep)}params_{GLOBAL_SETTINGS['model_dir_note']}"
...
print("[Optuna] Starting hyperparameter optimization for ANN...")
...
if study.best_trial:
    ...
    print("\n --- Best values: ---")
    print(f"batchsize_int: {batchsize_int}, densesize_int: {densesize_int}, windowsize_int: {windowsize_int}")
    print(f"learning_rate: {learning_rate:.6f}, dropout_rate: {dropout_rate:.2f}, n_layers: {n_layers}")
    
    # Save only the simple best parameters file
    best_params_simple = {
...
master_log_file = BASE_DIR / "model_runs" / "OptunaOpt" / "optimization_runs_log_ANN.json"
